...
namespace BikeDistributor.Domain
{
	public class OrderFactsAsync
	{
		...
		public async Task Get_All_Orders_By_Mountain_Bike_Shop_Company_Name_Async()
		{
			#region Arrange
			//Arrange
			//Instance of de In-Memory Database
			...
			#region Act
			//Act
			//Invoke helper to prepare mock stuff
			var mockSet = MoqExtension.ToDbSet<Order>(data);
			...
			var MountainBikeShopOrders = await Uow.OrderService().Find(x => x.Company.Equals("Mountain Bike Shop")).ToListAsync();
			#endregion
		...
		public async Task Get_Order_With_OrderId_Equal_Four_Async()
		{
			...
			var AlabamaBikeShopOrder = await Task.FromResult(Uow.OrderService().FindById(4));
			#endregion
...
Total: $16,087.50";
			...
			var _order = await Task.FromResult(Uow.OrderService().FindById(1));

			string textReceipt = await Task.FromResult(_order.Receipt());
			#endregion
...
Total: $9,652.50";
			...
			var _order = await Task.FromResult(Uow.OrderService().FindById(3));

			string ValleyBikeShopTextReceipt = await Task.FromResult(_order.Receipt());
			#endregion

			#region Assert
			//Assert
			//This time I'll use XUnit.Assert
			Assert.Equal(ResultStatementValleyBikeShopTextReceipt, ValleyBikeShopTextReceipt);
			#endregion
		}
		...
		public async Task Get_HtmlReceipt_For_Arktos_Bike_Async()
		{
			...
			var Uow = new UnitOfWork(mockContext.Object);
			var _order = await Uow.OrderService().Find(x => x.Company.Equals("Miami Bike Shop")).FirstOrDefaultAsync();

			string HtmlReceipt = await Task.FromResult(_order.HtmlReceipt());
			#endregion
